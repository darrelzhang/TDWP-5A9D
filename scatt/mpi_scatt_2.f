...
!====================================================================
      subroutine ExchangeData(iLoop) ! Based on two-sided communication.
      if(iLoop == 1) then
          call packRawData(iLoop) ! prepare raw data for this loop(1).
          call initExchange
          call FinishExchange
          call unpackRawData(iLoop) ! raw data for this loop(1) ready.
          call packRawData(iLoop+1) ! prepare raw data for next loop(2).
          call initExchange 
      else if(iLoop /= nLoop+1) then
          call finishExchange
          if(iLoop-2 >= 1) then 
              call unpackRstData(iLoop-2) ! store data updated@(iLoop-2)
          end if
          ...
          if(iLoop+1 <= nLoop) then     ! if any, 
              call packRawData(iLoop+1) ! prepare raw data for next loop
          end if
          call initExchange
      else
          call finishExchange
          call unpackRstData(iLoop-2) ! store data updated@(nLoop-1)
          call packRstData(iLoop-1) 
          call initExchange 
          call finishExchange
          call unpackRstData(iLoop-1) ! store data updated@(nLoop)
      endif
