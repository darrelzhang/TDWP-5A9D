...
!@nh4 pes lijun
	...
	use comparamd, only : r1ty,  zzty, betasty, gammasty,
     &                        thetaty, row1ty, row2ty 
        use nh3_bound_eigmd, only : nvbound, nrbound
        use nh4rotmd,   only : na1node, nb1node
        use nh4scattmd, only : idpot, nproc, idproc,  vmasy, vmint
        use smallmd
	implicit none
        integer :: nzasy, ierr
        integer :: i, j, k, l, ms
        integer :: nstart, nend, n5d
        ...
        if(idpot == 'asy') then ! asy
           ...
           print*, 'nzasy*nvbound=',nzasy*r1ty%nvb
           nstart=(nzasy*r1ty%nvb*row1ty%nvb*row2ty%nvb)*idproc/nproc+1
           nend  =(nzasy*r1ty%nvb*row1ty%nvb*row2ty%nvb)*(idproc+1)/nproc
           allocate(vmasy(n5d,(nend-nstart+1)),stat=ierr)

           print*,'nstart=',nstart,'nend=',nend
           call cal_pot_surf(nstart,nend,nzint+1,nz,zq(nzint+1),
     &                       r1ty%nvb,r1ty%rqa, n5d,vmasy)

          open(101,file='result/POT'//itoc4(idproc)//'.ASY',
     &         form='binary', status='unknown')
          do i=nstart, nend 
             write(101)(vmasy(j,i-nstart+1),j=1,n5d)
          enddo
          close(101)
        
          deallocate(vmasy, stat=ierr)
          print*,'pot asy finished!'   
          stop

        elseif(idpot == 'int') then !int

           nstart=(nzint*r1ty%nrb*row1ty%nvb*row2ty%nvb)*idproc/nproc+1
           nend  =(nzint*r1ty%nrb*row1ty%nvb*row2ty%nvb)*(idproc+1)/nproc
           allocate(vmint(n5d,(nend-nstart+1)),stat=ierr)
        
           call cal_pot_surf(nstart,nend,1, nzint,zq,r1ty%nrb,r1ty%rqi,
     &                       n5d, vmint)

          open(101,file='result/POT'//itoc4(idproc)//'.INT',
     &         form='binary',status='unknown')
          do i=nstart, nend 
             write(101)(vmint(j,i-nstart+1),j=1,n5d)
          enddo
          close(101)
        
          deallocate(vmint, stat=ierr)
          print*, 'pot int finished!'   
          stop
	...
	return
        ...
        subroutine cal_pot_surf(nstart,nend,nzs,nze,zq,nvb,rq,n5d, vmat)
	use comparamd, only : row1ty, row2ty, thetaty,
     &                        betasty, gammasty 
        use nh4rotmd,  only : na1node, nb1node, a1node, b1node
        implicit none
        integer :: nzvb
        integer :: irow2, irow1, ith, ig, ib, ir1, iz, ia1,ib1, nleft
        real(8) :: row2, row1, theta, gammas, betas, r1, zz, a1, b1, vx
        integer, intent(in)  :: nstart,nend,nzs,nze,nvb, n5d
        real(8), intent(in)  :: zq(nze-nzs+1), rq(nvb)
        real(4), intent(out) :: vmat(n5d,(nend-nstart+1))
        integer :: ms
        
        do nzvb=nstart, nend
           iz=(nzvb-1)/(nvb*row1ty%nvb*row2ty%nvb)+1     ; zz=zq(iz) 
           nleft=mod(nzvb-1,nvb*row1ty%nvb*row2ty%nvb)+1
           ir1=(nleft-1)/(row1ty%nvb*row2ty%nvb)+1       ; r1=rq(ir1) 
           nleft=mod(nleft-1,row1ty%nvb*row2ty%nvb)+1
           irow1=(nleft-1)/row2ty%nvb+1                  ; row1=row1ty%rqa(irow1)
           nleft=mod(nleft-1,row2ty%nvb)+1
           irow2=nleft                                   ; row2=row1ty%rqa(irow2)


           write(*,"('iz=',I3,1x,f10.6,1x,'ir1=',I3,1x,f10.6,1x,
     &              'irow1=',i3,1x,f10.6,1x,'irow2=',i3,1x,f10.6)") 
     &                iz,zz, ir1,r1, irow1,row1,irow2,row2
           
            ms=0
            do ib   =1, betasty%nvb  ; betas=betasty%rqa(ib)
            do ig   =1, gammasty%nvb ; gammas=gammasty%rqa(ig)
            do ith  =1, thetaty%nvb  ; theta=thetaty%rqa(ith)
            do ia1  =1, na1node      ; a1=a1node(ia1)
            do ib1  =1, nb1node      ; b1=b1node(ib1) 
               
	       call potdriv(zz,a1,b1,r1,betas,gammas, 
     &                               row1,row2,theta, vx)
               ms=ms+1
               vmat(ms,nzvb-nstart+1)=vx
